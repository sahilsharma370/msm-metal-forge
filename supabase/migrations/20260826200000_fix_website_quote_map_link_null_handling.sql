-- MSM Scrap — fix a blank optional map-link field blocking website Quote
-- submission.
--
-- Root cause: public.leads.seller_map_link and
-- buyer_destination_map_link each carry
--   check (<column> is null or <column> ~* '^https?://')
-- (20260811165757_create_quote_backend_foundation.sql). Both fields are
-- genuinely optional on the client (isValidMapLink treats a blank value as
-- valid, and the server's own submissionShapeSchema/normalizeSubmission
-- also accept an empty string through unchanged) — but this function's own
-- INSERT into public.leads wrote `p_submission ->> 'sellerMapLink'` /
-- `p_submission ->> 'buyerDestinationMapLink'` verbatim, unlike every
-- sibling numeric/date field in the same VALUES list (sellerQuantityValue,
-- sellerPickupDate, buyerQuantityValue, buyerRequiredByDate), which already
-- go through `nullif(..., '')` before their own cast. An empty string
-- satisfies neither branch of either CHECK constraint, so any submission
-- that leaves either optional map-link field blank raised
-- leads_seller_map_link_check / leads_buyer_destination_map_link_check and
-- rolled back the whole insert — surfaced to the customer as a generic
-- HTTP 500 "Something went wrong on our end", with no lead ever created.
--
-- Scope: a single, minimal CREATE OR REPLACE of create_website_quote_v1
-- (its CHECKPOINT C2F-C1 definition in
-- 20260814150000_expose_upload_slot_status.sql) wrapping exactly those two
-- VALUES-list expressions in `nullif(..., '')`, matching the exact pattern
-- already used for sellerQuantityValue/sellerPickupDate/buyerQuantityValue/
-- buyerRequiredByDate directly above/below them. Every other line —
-- validation, idempotent-replay handling, file-slot inserts, the
-- submission_snapshot jsonb (which deliberately keeps the raw as-submitted
-- value, including a blank string, for audit purposes — never normalized),
-- the zero-file complete_lead_if_ready call, the returned upload_slots
-- status field, atomicity, grants — is byte-for-byte unchanged from that
-- definition.
--
-- Neither CHECK constraint itself is touched, weakened or removed: a
-- genuinely invalid non-blank, non-URL value is still rejected exactly as
-- before. Only the missing NULL-normalization for an intentionally-blank
-- optional field is added, mirroring this same function's own existing
-- convention for its other optional fields.
--
...
begin
  ...
  begin
    ...
    values (
      ...
      p_submission ->> 'sellerEmirate', p_submission ->> 'sellerArea', nullif(p_submission ->> 'sellerMapLink', ''),
      ...
      nullif(p_submission ->> 'buyerDestinationMapLink', ''), p_submission ->> 'buyerFulfilment',
...
comment on function public.create_website_quote_v1(uuid, text, jsonb, jsonb) is
  ...
  'moment its last slot verifies. Each returned upload_slots element '
  'includes its authoritative current status '
  ...
  'never inferred. A blank optional sellerMapLink/buyerDestinationMapLink '
  'is stored as NULL (never an empty string), matching the CHECK '
  'constraint on both columns and this function''s own existing '
  'nullif(..., '''') convention for its other optional numeric/date '
  'fields; submission_snapshot still preserves the raw as-submitted value '
  'unchanged. SECURITY INVOKER — relies entirely on the caller''s '

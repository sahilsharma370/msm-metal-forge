...
export function QuotePhotoPicker({
  ...
  return (
    <div>
      ...
      {/* C2L (surgical contact photo fix) — the upload affordance is now one
          fixed ~128px square tile, with thumbnails continuing beside it in
          the same row, replacing the previous full-width oversized dropzone
          (large icon/title/browseLabel/acceptLine stack). browseLabel and
          acceptLine move to quiet helper text below the row instead of
          living inside the tile itself. */}
      <div className="mt-2 flex flex-wrap items-start gap-3">
        {!atCapacity && (
          <label
            ...
            className={`flex h-32 w-32 shrink-0 cursor-pointer flex-col items-center justify-center gap-1.5 rounded-xl border border-dashed px-2 text-center transition-colors ${
              dragActive
                ? "border-copper bg-[oklch(0.583_0.135_45.5/0.06)]"
                : "border-white/20 hover:border-copper/50"
            }`}
          >
            <Plus aria-hidden="true" className="h-4 w-4 text-foreground/50" />
            <span className="line-clamp-2 text-xs font-semibold text-foreground/80">
              {hasFiles ? "Add more" : title}
            </span>
          </label>
        )}
        ...
        {files.length > 0 && (
          <ul className="contents">
            {files.map((f) => (
              <li
                key={f.id}
                className="relative h-32 w-32 shrink-0 overflow-hidden rounded-xl border border-white/12 bg-white/5"
              >
                {f.previewUrl ? (
                  <img
                    src={f.previewUrl}
      ...
      {!atCapacity && (
        <p className="mt-2 text-[0.7rem] text-foreground/40">
          {browseLabel} · {acceptLine}
        </p>
      )}

...
/**
 * CHECKPOINT C2F-C: lives under src/lib (not src/server) for the same
 * reason canonicalize.ts does — see that file's doc comment. This module
 * defines both the server's request-validation rules (submissionSchema,
 * checkSubmissionCompleteness, initiateQuoteRequestSchema, file-declaration
 * rules) AND the pure shape/normalization primitives browser code needs
 * (submissionShapeSchema, normalizeSubmission, MAX_FILES) — kept in one
 * file rather than split, since the whole thing was already proven
 * browser-safe (only zod + Web Crypto + this project's own client-safe
 * quote-schema/quote-options/quote-search modules) and splitting it would
 * risk exactly the kind of drift this checkpoint is trying to prevent.
 * src/server/quote/submission-schema.ts re-exports everything from here so
 ...
 *
 * CHECKPOINT C2F-C: also the exact function browser code calls (via
 * src/components/site/quote/quote-submission-identity.ts) to compute what
 * the server would compute from the equivalent request — one
 * implementation, imported by both sides from this neutral module, never
 * duplicated.
 */
